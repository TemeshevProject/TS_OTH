...
$folder = $env:CU5000_FOLDER
$template = $env:CU5000_TEMPLATE
$output = $env:CU5000_OUTPUT
if (-not $folder -or -not $template -or -not $output) { throw 'Missing CU5000_FOLDER, CU5000_TEMPLATE or CU5000_OUTPUT environment variables' }
if (-not $env:CU5000_TRADE_NAME) { throw 'Missing CU5000_TRADE_NAME environment variable' }

Copy-Item -LiteralPath $template -Destination $output -Force

$componentsFile = if ($env:CU5000_COMPONENTS) { $env:CU5000_COMPONENTS } else { Join-Path $folder 'cu5000_components.json' }
$components = Get-Content $componentsFile -Raw -Encoding UTF8 | ConvertFrom-Json
$main = Get-Content (Join-Path $folder 'cu5000_main.json') -Raw -Encoding UTF8 | ConvertFrom-Json

$block1 = @{
    TradeName = $env:CU5000_TRADE_NAME
    ProducerName = 'CHAMMED CO., LTD.'
...
$section4File = if ($env:CU5000_SECTION4_FILE) { $env:CU5000_SECTION4_FILE } else { Join-Path $folder 'section4.txt' }
...
$block2Text = if ($env:CU5000_BLOCK2_FILE -and (Test-Path -LiteralPath $env:CU5000_BLOCK2_FILE)) {
    (Get-Content -LiteralPath $env:CU5000_BLOCK2_FILE -Raw -Encoding UTF8).Trim()
} else {
    $main.purpose
}
