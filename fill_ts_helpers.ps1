...
#   1. Связный текст (3–4 предложения) + маркированный список конкретных параметров (без «Тип», «Назначение», «Модель»).
#   2. Не указывать «Модель», «Модель/марка», производителя — идентификатор в Name.
#   3. Размеры и ключевые параметры из Name дублировать в списке; не повторять наименование основной техники в тексте.
