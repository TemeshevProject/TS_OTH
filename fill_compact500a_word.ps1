...
$folder = $env:COMPACT500A_FOLDER
$template = $env:COMPACT500A_TEMPLATE
$output = $env:COMPACT500A_OUTPUT
if (-not $folder -or -not $template -or -not $output) { throw 'Missing COMPACT500A_FOLDER, COMPACT500A_TEMPLATE or COMPACT500A_OUTPUT environment variables' }
if (-not $env:COMPACT500A_TRADE_NAME) { throw 'Missing COMPACT500A_TRADE_NAME environment variable' }

Copy-Item -LiteralPath $template -Destination $output -Force

$componentsFile = if ($env:COMPACT500A_COMPONENTS) { $env:COMPACT500A_COMPONENTS } else { Join-Path $folder 'compact500a_components.json' }
$components = Get-Content $componentsFile -Raw -Encoding UTF8 | ConvertFrom-Json
$main = Get-Content (Join-Path $folder 'compact500a_main.json') -Raw -Encoding UTF8 | ConvertFrom-Json

$block1 = @{
    TradeName = $env:COMPACT500A_TRADE_NAME
    ProducerName = 'Shenzhen Browiner Tech Co., Ltd'
...
$section4File = if ($env:COMPACT500A_SECTION4_FILE) { $env:COMPACT500A_SECTION4_FILE } else { Join-Path $folder 'section4.txt' }
...
$block2Text = if ($env:COMPACT500A_BLOCK2_FILE -and (Test-Path -LiteralPath $env:COMPACT500A_BLOCK2_FILE)) {
    (Get-Content -LiteralPath $env:COMPACT500A_BLOCK2_FILE -Raw -Encoding UTF8).Trim()
} else {
    $main.purpose
}
