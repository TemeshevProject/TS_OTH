...
$folder = $env:FACLYRIC_KIT2_FOLDER
$template = $env:FACLYRIC_KIT2_TEMPLATE
$output = $env:FACLYRIC_KIT2_OUTPUT
if (-not $folder -or -not $template -or -not $output) { throw 'Missing FACLYRIC_KIT2_FOLDER, FACLYRIC_KIT2_TEMPLATE or FACLYRIC_KIT2_OUTPUT environment variables' }
if (-not $env:FACLYRIC_KIT2_TRADE_NAME) { throw 'Missing FACLYRIC_KIT2_TRADE_NAME environment variable' }

Copy-Item -LiteralPath $template -Destination $output -Force

$componentsFile = if ($env:FACLYRIC_KIT2_COMPONENTS) { $env:FACLYRIC_KIT2_COMPONENTS } else { Join-Path $folder 'faclyric_kit2_components.json' }
...
$block1 = @{
    TradeName = $env:FACLYRIC_KIT2_TRADE_NAME
...
$section4File = if ($env:FACLYRIC_KIT2_SECTION4_FILE) { $env:FACLYRIC_KIT2_SECTION4_FILE } else { Join-Path $folder 'section4.txt' }
...
$block2Text = if ($env:FACLYRIC_KIT2_BLOCK2_FILE -and (Test-Path -LiteralPath $env:FACLYRIC_KIT2_BLOCK2_FILE)) {
    (Get-Content -LiteralPath $env:FACLYRIC_KIT2_BLOCK2_FILE -Raw -Encoding UTF8).Trim()
} else {
    $main.purpose
}
