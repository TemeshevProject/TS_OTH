...
$folder = $env:LOGIQP9_FOLDER
$template = $env:LOGIQP9_TEMPLATE
$output = $env:LOGIQP9_OUTPUT
if (-not $folder -or -not $template -or -not $output) { throw 'Missing LOGIQP9_FOLDER, LOGIQP9_TEMPLATE or LOGIQP9_OUTPUT environment variables' }
if (-not $env:LOGIQP9_TRADE_NAME) { throw 'Missing LOGIQP9_TRADE_NAME environment variable' }

Copy-Item -LiteralPath $template -Destination $output -Force

$componentsFile = if ($env:LOGIQP9_COMPONENTS) { $env:LOGIQP9_COMPONENTS } else { Join-Path $folder 'logiqp9_components.json' }
$components = Get-Content $componentsFile -Raw -Encoding UTF8 | ConvertFrom-Json
$main = Get-Content (Join-Path $folder 'logiqp9_main.json') -Raw -Encoding UTF8 | ConvertFrom-Json

$block1 = @{
    TradeName = $env:LOGIQP9_TRADE_NAME
    ProducerName = 'GE Ultrasound Korea, Ltd.'
...
$section4File = if ($env:LOGIQP9_SECTION4_FILE) { $env:LOGIQP9_SECTION4_FILE } else { Join-Path $folder 'section4.txt' }
...
$block2Text = if ($env:LOGIQP9_BLOCK2_FILE -and (Test-Path -LiteralPath $env:LOGIQP9_BLOCK2_FILE)) {
    (Get-Content -LiteralPath $env:LOGIQP9_BLOCK2_FILE -Raw -Encoding UTF8).Trim()
} else {
    $main.purpose
}
