...
(async () => {
  const { main, complect } = await fetchRegistryData({
    regNumber: 'РК МИ (МТ)-0№027833',
    searchDigits: '027833',
    searchText: 'EXTRON 5',
  });

  fs.writeFileSync(path.join(folder, 'extron5_main.json'), JSON.stringify(main, null, 2), 'utf8');
  fs.writeFileSync(path.join(folder, 'extron5_complect.json'), JSON.stringify(complect, null, 2), 'utf8');
