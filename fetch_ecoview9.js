...
(async () => {
  const { main, complect } = await fetchRegistryData({
    regNumber: 'РК-МТ-7№010772',
    searchDigits: '010772',
    searchText: 'EcoView 9',
  });

  fs.writeFileSync(path.join(folder, 'ecoview9_main.json'), JSON.stringify(main, null, 2), 'utf8');
  fs.writeFileSync(path.join(folder, 'ecoview9_complect.json'), JSON.stringify(complect, null, 2), 'utf8');
