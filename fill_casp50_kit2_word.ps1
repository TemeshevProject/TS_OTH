...
$folder = $env:CASP50_KIT2_FOLDER
$template = $env:CASP50_KIT2_TEMPLATE
$output = $env:CASP50_KIT2_OUTPUT
if (-not $folder -or -not $template -or -not $output) { throw 'Missing CASP50_KIT2_FOLDER, CASP50_KIT2_TEMPLATE or CASP50_KIT2_OUTPUT environment variables' }
if (-not $env:CASP50_KIT2_TRADE_NAME) { throw 'Missing CASP50_KIT2_TRADE_NAME environment variable' }

Copy-Item -LiteralPath $template -Destination $output -Force

$componentsFile = if ($env:CASP50_KIT2_COMPONENTS) { $env:CASP50_KIT2_COMPONENTS } else { Join-Path $folder 'casp50_kit2_components.json' }
$components = Get-Content $componentsFile -Raw -Encoding UTF8 | ConvertFrom-Json
$main = Get-Content (Join-Path $folder 'casp50_main.json') -Raw -Encoding UTF8 | ConvertFrom-Json

$block1 = @{
    TradeName = $env:CASP50_KIT2_TRADE_NAME
    ProducerName = 'MuDanJiang Plasma Physical Application Technology Co., Ltd'
...
$section4File = if ($env:CASP50_KIT2_SECTION4_FILE) { $env:CASP50_KIT2_SECTION4_FILE } else { Join-Path $folder 'section4.txt' }
...
$block2Text = if ($env:CASP50_KIT2_BLOCK2_FILE -and (Test-Path -LiteralPath $env:CASP50_KIT2_BLOCK2_FILE)) {
    (Get-Content -LiteralPath $env:CASP50_KIT2_BLOCK2_FILE -Raw -Encoding UTF8).Trim()
} else {
    $main.purpose
}
