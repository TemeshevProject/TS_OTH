...
$folder = $env:UCT550_KIT2_FOLDER
$template = $env:UCT550_KIT2_TEMPLATE
$output = $env:UCT550_KIT2_OUTPUT
if (-not $folder -or -not $template -or -not $output) { throw 'Missing UCT550_KIT2_FOLDER, UCT550_KIT2_TEMPLATE or UCT550_KIT2_OUTPUT environment variables' }
if (-not $env:UCT550_KIT2_TRADE_NAME) { throw 'Missing UCT550_KIT2_TRADE_NAME environment variable' }

Copy-Item -LiteralPath $template -Destination $output -Force

$componentsFile = if ($env:UCT550_KIT2_COMPONENTS) { $env:UCT550_KIT2_COMPONENTS } else { Join-Path $folder 'uct550_kit2_components.json' }
...
$block1 = @{
    TradeName = $env:UCT550_KIT2_TRADE_NAME
...
$section4File = if ($env:UCT550_KIT2_SECTION4_FILE) { $env:UCT550_KIT2_SECTION4_FILE } else { Join-Path $folder 'section4.txt' }
...
$block2Text = if ($env:UCT550_KIT2_BLOCK2_FILE -and (Test-Path -LiteralPath $env:UCT550_KIT2_BLOCK2_FILE)) {
    (Get-Content -LiteralPath $env:UCT550_KIT2_BLOCK2_FILE -Raw -Encoding UTF8).Trim()
} else {
    $main.purpose
}
