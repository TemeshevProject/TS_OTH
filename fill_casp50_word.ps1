...
$folder = $env:CASP50_FOLDER
$template = $env:CASP50_TEMPLATE
$output = $env:CASP50_OUTPUT
if (-not $folder -or -not $template -or -not $output) { throw 'Missing CASP50_FOLDER, CASP50_TEMPLATE or CASP50_OUTPUT environment variables' }
if (-not $env:CASP50_TRADE_NAME) { throw 'Missing CASP50_TRADE_NAME environment variable' }

Copy-Item -LiteralPath $template -Destination $output -Force

$componentsFile = if ($env:CASP50_COMPONENTS) { $env:CASP50_COMPONENTS } else { Join-Path $folder 'casp50_components.json' }
...
$block1 = @{
    TradeName = $env:CASP50_TRADE_NAME
...
$section4File = if ($env:CASP50_SECTION4_FILE) { $env:CASP50_SECTION4_FILE } else { Join-Path $folder 'section4.txt' }
...
$block2Text = if ($env:CASP50_BLOCK2_FILE -and (Test-Path -LiteralPath $env:CASP50_BLOCK2_FILE)) {
    (Get-Content -LiteralPath $env:CASP50_BLOCK2_FILE -Raw -Encoding UTF8).Trim()
} else {
    $main.purpose
}
