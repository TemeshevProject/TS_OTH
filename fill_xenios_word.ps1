...
$folder = $env:XENIOS_FOLDER
$template = $env:XENIOS_TEMPLATE
$output = $env:XENIOS_OUTPUT
if (-not $folder -or -not $template -or -not $output) { throw 'Missing XENIOS_FOLDER, XENIOS_TEMPLATE or XENIOS_OUTPUT environment variables' }
if (-not $env:XENIOS_TRADE_NAME) { throw 'Missing XENIOS_TRADE_NAME environment variable' }

Copy-Item -LiteralPath $template -Destination $output -Force

$componentsFile = if ($env:XENIOS_COMPONENTS) { $env:XENIOS_COMPONENTS } else { Join-Path $folder 'xenios_components.json' }
$components = Get-Content $componentsFile -Raw -Encoding UTF8 | ConvertFrom-Json
$main = Get-Content (Join-Path $folder 'xenios_main.json') -Raw -Encoding UTF8 | ConvertFrom-Json

$block1 = @{
    TradeName = $env:XENIOS_TRADE_NAME
    ProducerName = 'Xenios AG'
...
$section4File = if ($env:XENIOS_SECTION4_FILE) { $env:XENIOS_SECTION4_FILE } else { Join-Path $folder 'section4.txt' }
...
$block2Text = if ($env:XENIOS_BLOCK2_FILE -and (Test-Path -LiteralPath $env:XENIOS_BLOCK2_FILE)) {
    (Get-Content -LiteralPath $env:XENIOS_BLOCK2_FILE -Raw -Encoding UTF8).Trim()
} else {
    $main.purpose
}
