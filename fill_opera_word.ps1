...
$folder = $env:OPERA_FOLDER
$template = $env:OPERA_TEMPLATE
$output = $env:OPERA_OUTPUT
if (-not $folder -or -not $template -or -not $output) { throw 'Missing OPERA_FOLDER, OPERA_TEMPLATE or OPERA_OUTPUT environment variables' }
if (-not $env:OPERA_TRADE_NAME) { throw 'Missing OPERA_TRADE_NAME environment variable' }

Copy-Item -LiteralPath $template -Destination $output -Force

$componentsFile = if ($env:OPERA_COMPONENTS) { $env:OPERA_COMPONENTS } else { Join-Path $folder 'opera_components.json' }
$components = Get-Content $componentsFile -Raw -Encoding UTF8 | ConvertFrom-Json
$main = Get-Content (Join-Path $folder 'opera_main.json') -Raw -Encoding UTF8 | ConvertFrom-Json

$block1 = @{
    TradeName = $env:OPERA_TRADE_NAME
...
$section4File = if ($env:OPERA_SECTION4_FILE) { $env:OPERA_SECTION4_FILE } else { Join-Path $folder 'section4.txt' }
...
$block2Text = if ($env:OPERA_BLOCK2_FILE -and (Test-Path -LiteralPath $env:OPERA_BLOCK2_FILE)) {
    (Get-Content -LiteralPath $env:OPERA_BLOCK2_FILE -Raw -Encoding UTF8).Trim()
} else {
    $main.purpose
}
