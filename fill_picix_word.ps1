...
$folder = $env:PICIX_FOLDER
$template = $env:PICIX_TEMPLATE
$output = $env:PICIX_OUTPUT
if (-not $folder -or -not $template -or -not $output) { throw 'Missing PICIX_FOLDER, PICIX_TEMPLATE or PICIX_OUTPUT environment variables' }
if (-not $env:PICIX_TRADE_NAME) { throw 'Missing PICIX_TRADE_NAME environment variable' }

Copy-Item -LiteralPath $template -Destination $output -Force

$componentsFile = if ($env:PICIX_COMPONENTS) { $env:PICIX_COMPONENTS } else { Join-Path $folder 'picix_components.json' }
$components = Get-Content $componentsFile -Raw -Encoding UTF8 | ConvertFrom-Json
$main = Get-Content (Join-Path $folder 'picix_main.json') -Raw -Encoding UTF8 | ConvertFrom-Json

$block1 = @{
    TradeName = $env:PICIX_TRADE_NAME
...
$section4File = if ($env:PICIX_SECTION4_FILE) { $env:PICIX_SECTION4_FILE } else { Join-Path $folder 'section4.txt' }
...
$block2Text = if ($env:PICIX_BLOCK2_FILE -and (Test-Path -LiteralPath $env:PICIX_BLOCK2_FILE)) {
    (Get-Content -LiteralPath $env:PICIX_BLOCK2_FILE -Raw -Encoding UTF8).Trim()
} else {
    $main.purpose
}
