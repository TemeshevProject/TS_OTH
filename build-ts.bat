...
echo   2. A7
echo   3. Navigator DR Care
echo   4. HyLED C8
echo   5. ENDODRY
echo.
echo   0. Выход
echo.
set /p CHOICE="   Выберите комплект (0-5): "

if "%CHOICE%"=="1" set "TARGET=giotto" & goto :run
if "%CHOICE%"=="2" set "TARGET=a7" & goto :run
if "%CHOICE%"=="3" set "TARGET=navigator" & goto :run
if "%CHOICE%"=="4" set "TARGET=hyled" & goto :run
if "%CHOICE%"=="5" set "TARGET=endodry" & goto :run
