...
$folder = $env:PERFOX_FOLDER
$template = $env:PERFOX_TEMPLATE
$output = $env:PERFOX_OUTPUT
if (-not $folder -or -not $template -or -not $output) { throw 'Missing PERFOX_FOLDER, PERFOX_TEMPLATE or PERFOX_OUTPUT environment variables' }
if (-not $env:PERFOX_TRADE_NAME) { throw 'Missing PERFOX_TRADE_NAME environment variable' }

Copy-Item -LiteralPath $template -Destination $output -Force

$componentsFile = if ($env:PERFOX_COMPONENTS) { $env:PERFOX_COMPONENTS } else { Join-Path $folder 'perfox_components.json' }
$components = Get-Content $componentsFile -Raw -Encoding UTF8 | ConvertFrom-Json
$main = Get-Content (Join-Path $folder 'perfox_main.json') -Raw -Encoding UTF8 | ConvertFrom-Json

$block1 = @{
    TradeName = $env:PERFOX_TRADE_NAME
...
$section4File = if ($env:PERFOX_SECTION4_FILE) { $env:PERFOX_SECTION4_FILE } else { Join-Path $folder 'section4.txt' }
...
$block2Text = if ($env:PERFOX_BLOCK2_FILE -and (Test-Path -LiteralPath $env:PERFOX_BLOCK2_FILE)) {
    (Get-Content -LiteralPath $env:PERFOX_BLOCK2_FILE -Raw -Encoding UTF8).Trim()
} else {
    $main.purpose
}
