...
$folder = $env:A7_FOLDER
$template = $env:A7_TEMPLATE
$output = $env:A7_OUTPUT
if (-not $folder -or -not $template -or -not $output) { throw 'Missing A7_FOLDER, A7_TEMPLATE or A7_OUTPUT environment variables' }
if (-not $env:A7_TRADE_NAME) { throw 'Missing A7_TRADE_NAME environment variable' }

Copy-Item -LiteralPath $template -Destination $output -Force

$componentsFile = if ($env:A7_COMPONENTS) { $env:A7_COMPONENTS } else { Join-Path $folder 'a7_components.json' }
$components = Get-Content $componentsFile -Raw -Encoding UTF8 | ConvertFrom-Json
$main = Get-Content (Join-Path $folder 'a7_main.json') -Raw -Encoding UTF8 | ConvertFrom-Json

$block1 = @{
    TradeName = $env:A7_TRADE_NAME
...
$section4File = if ($env:A7_SECTION4_FILE) { $env:A7_SECTION4_FILE } else { Join-Path $folder 'section4.txt' }
...
$block2Text = if ($env:A7_BLOCK2_FILE -and (Test-Path -LiteralPath $env:A7_BLOCK2_FILE)) {
    (Get-Content -LiteralPath $env:A7_BLOCK2_FILE -Raw -Encoding UTF8).Trim()
} else {
    $main.purpose
}
