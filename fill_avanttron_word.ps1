...
$folder = $env:AVANTTRON_FOLDER
$template = $env:AVANTTRON_TEMPLATE
$output = $env:AVANTTRON_OUTPUT
if (-not $folder -or -not $template -or -not $output) { throw 'Missing AVANTTRON_FOLDER, AVANTTRON_TEMPLATE or AVANTTRON_OUTPUT environment variables' }
if (-not $env:AVANTTRON_TRADE_NAME) { throw 'Missing AVANTTRON_TRADE_NAME environment variable' }

Copy-Item -LiteralPath $template -Destination $output -Force

$componentsFile = if ($env:AVANTTRON_COMPONENTS) { $env:AVANTTRON_COMPONENTS } else { Join-Path $folder 'avanttron_components.json' }
$components = Get-Content $componentsFile -Raw -Encoding UTF8 | ConvertFrom-Json
$main = Get-Content (Join-Path $folder 'avanttron_main.json') -Raw -Encoding UTF8 | ConvertFrom-Json

$block1 = @{
    TradeName = $env:AVANTTRON_TRADE_NAME
...
$section4File = if ($env:AVANTTRON_SECTION4_FILE) { $env:AVANTTRON_SECTION4_FILE } else { Join-Path $folder 'section4.txt' }
...
$block2Text = if ($env:AVANTTRON_BLOCK2_FILE -and (Test-Path -LiteralPath $env:AVANTTRON_BLOCK2_FILE)) {
    (Get-Content -LiteralPath $env:AVANTTRON_BLOCK2_FILE -Raw -Encoding UTF8).Trim()
} else {
    $main.purpose
}
